...
```bash
curl -s "$BASE/admin/logs?limit=50" -H "$M"            # 逐条请求日志
...
| 观测 | `GET /admin/logs`、`GET /admin/usage`、`GET /admin/deployment-health` |
...
进入 **06 Traffic**：查看逐条请求日志与按 alias 的用量/成本汇总，确认流量在多个 Deployment 间的分布是否符合所选策略。
