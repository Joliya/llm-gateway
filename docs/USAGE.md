...
- **路由落点**:每条日志记录负载均衡/故障转移后**实际命中的供应商**——`provider_name`(具体供应商名)+ `credential_id`(被选中的那条凭据)+ `provider_type`(适配器类型)。同一 alias 跨多凭据轮询时,可据此看出每次请求究竟走了哪条上游。控制台日志列表的「供应商」列显示 `provider_name`,详情里还会带上凭据编号。
...
**成本响应头(对接 Opik 等客户端侧观测)**:非流式成功响应会带上 `x-litellm-response-cost: <usd>`(开关 `GW_COST_HEADER_ENABLED`,默认 `true`),格式与 litellm 代理一致。

- 作用:litellm 系客户端(包括 `dspy.LM`)会把所有响应头转发进 `response._hidden_params["additional_headers"]["llm_provider-x-litellm-response-cost"]`,litellm SDK 据此填 `response_cost` → 进入 DSPy `lm.history[-1]["cost"]` → `OpikCallback` 上报为 span 的 `total_cost`。**客户端零改动**即可让网关算出的成本流入 Opik。
- 范围:`/v1/chat/completions`(非流式)、`/v1/completions`、`/v1/embeddings`。**流式不带**该头——成本只有在流结束后才算得出,而响应头早已发出;流式客户端通常按最后一帧的 usage 在本地估算。
- 介意把成本基准暴露给调用方时设 `GW_COST_HEADER_ENABLED=false`。
...
| 设置 | `GET /admin/settings`、`PUT /admin/settings/{key}`、`POST /admin/settings/currency/refresh`(立即拉取汇率) |

---

### 1.8 设置 / 汇率与展示币种

控制台所有金额（成本、花费、预算）都以**美元(USD)记账**。`Settings` 页面可设置汇率把美元换算成其他币种展示——这只影响**展示**，不改变记账与计费。

```bash
# 1 USD = 7.15 CNY、0.92 EUR；控制台用人民币展示金额
curl -s -X PUT $BASE/admin/settings/currency -H "$M" -H "$J" -d '{
  "rates": {"CNY": 7.15, "EUR": 0.92},
  "display": "CNY"
}'
```

- 币种代码自动转大写；`display` 必须是 `USD` 或 `rates` 里已有的币种，汇率须为正数，否则 `400`。
- `display: "USD"` 表示不换算、显示原始美元金额。
- 所有表格里的金额按展示币种换算并保留 **5 位小数**，鼠标悬停可看到完整精度金额（以及对应的原始美元值）。
- 设置以 key→JSON 形式存储（`settings` 表），后续新增设置项无需改表结构。

**汇率每日自动更新**:后台任务每天从免费公共接口拉取一次美元对各币种的汇率,**优先 exchangerate-api**(`open.er-api.com`,免 Key),失败则降级到 Frankfurter、exchangerate.host。

- 只刷新你**已配置**的币种,不会新增币种、也不改 `display`;某个源没覆盖到的币种保留原值。
- 启动时会做一次「节流刷新」(若距上次更新不足一个周期则跳过,避免重启时频繁打接口或覆盖刚手动改的值),之后每 `GW_FX_UPDATE_INTERVAL` 秒刷新一次。
- 想立刻刷新(无需等一天):`Settings` 页点「立即刷新汇率」,或 `POST /admin/settings/currency/refresh`。
- 相关环境变量:`GW_FX_AUTO_UPDATE`(默认 `true`,设 `false` 关闭自动更新)、`GW_FX_UPDATE_INTERVAL`(默认 `86400` 秒 = 1 天)、`GW_FX_FETCH_TIMEOUT`(默认 `15` 秒)。
